...
export const ThemeManagerProvider: React.FC<ThemeManagerProviderProps> = ({
  ...
  const allThemes = React.useMemo(() => [...presets, ...customThemes], [presets, customThemes]);
  ...
  const [editorState, setEditorState] = React.useState<EditorState>({ open: false });
  ...
  const openEditor = React.useCallback(
      (theme?: ThemeDefinition) => {
        if (theme) {
          ...
          const baseTheme = presets[0] || activeThemeDef;
          const newIdBase = baseTheme.name.toLowerCase().replace(/\s+/g, "-");
          const newId = `${newIdBase}-${Date.now()}`;
          setEditorState({
            open: true,
  ...
  const updateDraft = React.useCallback((updates: Partial<ThemeDefinition>) => {
    setEditorState((prev) => ({
      ...prev,
      draft: prev.draft ? { ...prev.draft, ...updates } : prev.draft,
    }));
  }, []);

  const setDraftFromThemeOptionsJson = React.useCallback((json: string) => {
    setEditorState((prev) => {
      ...
      try {
        ...
        return {
          ...prev,
          draft: { ...prev.draft, themeOptions: parsed },
          jsonError: undefined,
        };
      } catch (err) {
        const message = err instanceof Error ? err.message : "Invalid JSON";
        return { ...prev, jsonError: message };
      }
    });
  }, []);
